...
import { ExternalLink, FilePlus2, FileUp, ImagePlus, LayoutDashboard, Save, Settings2 } from "lucide-react";
...
export function EditorApp({ initialArticles, editorEmail, signOutPath }: { initialArticles: EditableArticle[]; editorEmail: string; signOutPath: string }) {
  ...
  function selectArticle(article: EditableArticle) {
    setForm({ id: article.id, title: article.title, kicker: article.kicker, subtitle: article.subtitle, slug: article.slug, excerpt: article.excerpt, summary: article.summary, content: article.content, category: article.category, tags: article.tags, coverImage: article.coverImage, imageAlt: article.imageAlt, status: article.status });
    setMessage("");
  }
  ...
  async function save(event: React.FormEvent) {
    ...
    setMessage("");
    try {
      const response = await fetch(editing ? `/api/articles/${form.id}` : "/api/articles", { method: editing ? "PUT" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(form) });
      ...
      setArticles((current) => editing ? current.map((article) => article.id === saved.id ? saved : article) : [saved, ...current]);
      selectArticle(saved);
      setMessage(saved.status === "published" ? "Artikel gespeichert und veröffentlicht." : "Entwurf gespeichert.");
    } catch (error) {
    ...
    <section className="editor-workspace">
      ...
      <div className="page-shell editor-layout">
        ...
        <form onSubmit={save} className="document-editor">
          <div className="document-editor-topbar"><span>{editing ? "Artikel bearbeiten" : "Neuer Artikel"}</span>{editing && form.status === "published" && <a href={`/artikel/${form.slug}`} target="_blank" rel="noreferrer">Live ansehen <ExternalLink /></a>}</div>
          ...
          <footer className="document-savebar"><p className={isErrorMessage(message) ? "error" : ""} role="status">{message || "Änderungen werden erst beim Speichern übernommen."}</p><Button type="submit" disabled={saving || uploading || importing}><Save />{saving ? "Wird gespeichert …" : editing ? "Änderungen speichern" : "Dossier speichern"}</Button></footer>
        </form>
      </div>
    </section>
  );
