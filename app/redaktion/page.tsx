...
export default async function RedaktionPage() {
  ...
  const editable: EditableArticle[] = rows.map((row) => ({ id: row.id, title: row.title, kicker: row.kicker, subtitle: row.subtitle, slug: row.slug, excerpt: row.excerpt, summary: row.summary, content: legacyTextToHtml(row.content), category: row.category, tags: parseTags(row.tags).join(", "), coverImage: row.coverImage ?? "", imageAlt: row.imageAlt ?? "", status: row.status as "draft" | "published", updatedAt: row.updatedAt }));
