...
Write-Host "[.ABSEITS 0.6.1] Abhängigkeiten werden geprüft ..."
npm install --no-audit --no-fund
if ($LASTEXITCODE -ne 0) {
    throw "npm install ist mit Exitcode $LASTEXITCODE fehlgeschlagen."
}

Write-Host "[.ABSEITS 0.6.1] Startet unter http://localhost:$Port"
Write-Host "[.ABSEITS 0.6.1] Redaktion: http://localhost:$Port/redaktion"
Write-Host "[.ABSEITS 0.6.1] Beim ersten Aufruf wird das Redaktionskonto eingerichtet."
